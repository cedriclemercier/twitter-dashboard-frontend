...
export default function Nav({ openNav, onCloseNav, setSearchResults }) {
  ...
  const loadAccounts = useCallback(async () => {
    ...
    const { userId } = auth;
    try {
      accounts = await fetch(`${process.env.REACT_APP_API}/api/users/accounts?userId=${userId}`)
  ...
  const renderContent = (
    ...
    >
      ...
      {followedAccounts && auth.userId !== "false" && <NavSection data={navConfig(followedAccounts)} setSearchResults={setSearchResults} />
      }
